...
**Status:** implemented for HTML variants (v0.1); hosted `badge.svg` remains a separate track.
...
- [x] Three HTML variants, each fully inline-styled, each opening with `all:unset;`.
- [x] Each renders correctly on light AND dark backgrounds from a single tag.
- [x] Alignment fix implemented per the Alignment section (flex-centered monogram; mini-card two-line sibling). Spot-check at 100%/200% zoom when changing markup.
